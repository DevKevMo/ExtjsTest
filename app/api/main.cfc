<cfcomponent>
    ...
    <cffunction name="login" access="remote" httpmethod="GET" returnFormat="JSON" description="set Session Id" auth="Kevin Moritz">
        <cfargument name="name" default="{username:"",password:""}" hint="values for username and password to set the SessionId" required="yes"
            type="strings" />
                ...
                <cfset returnStr.error=true>
                    <cfset returnStr.message="error: no user found">
                        <cfset returnStr.Data="">
                            ...
                            <cfdump var="#returnStr#" format="html" output="#expandPath('./returnStr_dump.html')#" />
                            <cfreturn returnStr>
    </cffunction>
